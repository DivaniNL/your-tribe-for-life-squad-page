...
<li class="person-card">
    {#if member.mugshot}
        <picture>
            ...
            <!-- als member.id gelijk is aan 30 dan krijgt die img fetch priority high -->
            <img class="animation-scale-out" src="https://fdnd.directus.app/assets/{member.mugshot}?width=500&height=500" alt="foto van {member.name}" style="view-transition-name: person-{member.id}" { ...(member.id === 30 ? { fetchpriority: 'high' } : {}) }>
        </picture>
    {:else}
